"""
Approval audit trail empty-state E2E — APPROVED tab variant.

Sibling of approval-audit-empty.spec.py, but targets the "معتمَدة"
(Approved) tab of /dashboard/expenses/review. Verifies that when a
claim that already reached `approved` has zero audit_log rows (e.g.
legacy import, purged retention), the ApprovalAuditTrail card renders
the empty state cleanly in RTL — no spinner, no error, no ghost
timeline.

Flow:
  1. Sign in owner + super_admin (AAL2).
  2. Owner registers a company and creates one claim.
  3. Super_admin PATCHes the claim straight to `approved` via REST.
  4. Every audit_log row for that claim is purged via psql.
  5. Reviewer opens /dashboard/expenses/review, switches to the
     "Approved" tab, expands the row's audit trail, and asserts:
       - the tab currently active is the Approved one,
       - `approvalAudit.empty` copy is visible,
       - zero `<li>` events rendered inside `ol.border-s`,
       - no loading / error copy visible,
       - `html[dir=rtl]` + `lang=ar`,
       - the empty-state paragraph's `text-align` computes to `start`
         or `right` (RTL alignment) so it doesn't leak LTR alignment.

Skips (exit 0) when TEST_SEED_TOKEN or PGHOST is missing.

Run:  python3 tests/e2e/approval-audit-empty-approved.spec.py
"""
import asyncio
...
SHOTS = Path("/tmp/browser/approval-audit-empty-approved")
    ...
    async with async_playwright() as pw:
        ...
        try:
            ...
            reg = await rest(o_page, "POST", "/rest/v1/rpc/register_company",
                             o_tok["access_token"],
                             body={"_name": "E2E Audit Empty Approved Co",
                                   "_phone": "0500000097"})
            ...
            slug = f"EMPTA-{os.getpid()}"
            create = await rest(
                o_page, "POST", "/rest/v1/expense_claims",
                o_tok["access_token"],
                body={"org_id": org_id, "submitted_by": o_tok["user_id"],
                      "title": "Audit empty-approved claim",
                      "amount": 88.00, "currency": "SAR",
            ...
            # Switch to the Approved tab (AR first, EN fallback).
            try:
            ...
            # Verify the active tab really is "Approved".
            active_tab_state = await a_page.evaluate("""
              () => {
                const el = document.querySelector('[role="tab"][data-state="active"]');
                return el ? el.textContent.trim() : null;
              }
            """)
            tab_ok = active_tab_state and (
                APPROVED_TAB_AR in active_tab_state
                or APPROVED_TAB_EN in active_tab_state
            )
            rs.report("Approved tab is active",
                      bool(tab_ok), f"active_tab={active_tab_state!r}")
            if not tab_ok:
                failures += 1

            await a_page.wait_for_selector(f"text=EC-{slug}", timeout=8000)

            # Expand the row's history.
            await a_page.get_by_role(
            ...
            rs.report("empty-state copy visible in Approved tab",
            ...
            # RTL alignment sanity — the empty-state paragraph must be
            # centred (text-align:center) OR flow from the RTL start
            # side ("start"/"right"). If a stray LTR utility landed on
            # it we'd see "left" here and the copy would look wrong
            # in AR.
            align = await a_page.evaluate(
                """
                (needleAr, needleEn) => {
                  const nodes = Array.from(document.querySelectorAll(
                    'ol.border-s ~ *, [class*="border-s"] ~ *, .text-center'
                  ));
                  const el = Array.from(document.querySelectorAll('*'))
                    .find(n => (n.textContent||'').trim() === needleAr
                            || (n.textContent||'').trim() === needleEn);
                  return el ? getComputedStyle(el).textAlign : null;
                }
                """,
                EMPTY_COPY_AR, EMPTY_COPY_EN,
            )
            align_ok = align in ("center", "start", "right")
            rs.report("empty-state alignment is RTL-safe",
                      align_ok, f"text-align={align!r}")
            if not align_ok:
                failures += 1

            await a_page.screenshot(
                path=str(SHOTS / "empty_trail_approved_rtl.png"))
