...
// ---------------- Commissions ----------------
// Note: DB `commission_status` enum = 'pending' | 'invoiced' | 'paid'.
// `deal_id` and `created_by` are NOT NULL in the schema.
...
export const updateCommission = createServerFn({ method: "POST" })
  ...
  .inputValidator((i) =>
    z
      .object({ id: z.string().uuid() })
      .merge(commissionInput.partial().omit({ org_id: true, deal_id: true }))
      .parse(i),
  )
  .handler(async ({ data, context }) => {
    const { id, ...patch } = data;
