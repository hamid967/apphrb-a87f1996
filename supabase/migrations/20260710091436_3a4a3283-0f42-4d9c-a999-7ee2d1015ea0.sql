...
-- Notifications for policy violations: notify claim submitter and pending approvers
...
BEGIN
  ...
  IF v_claim.submitted_by IS NOT NULL THEN
    INSERT INTO public.notification_queue (
      ...
      'pv:' || NEW.id::text || ':sub:' || v_claim.submitted_by::text
    )
    ON CONFLICT (idempotency_key) DO NOTHING;
  END IF;

  -- Notify pending approvers (those whose level is current or upcoming and not yet decided)
  FOR v_approver IN
  ...
  LOOP
    INSERT INTO public.notification_queue (
      ...
      v_approver.user_id,
      'pv:' || NEW.id::text || ':apr:' || v_approver.user_id::text
    )
    ON CONFLICT (idempotency_key) DO NOTHING;
  END LOOP;
