...
-- Monthly billing series (revenue + paying orgs + new/churned)
CREATE OR REPLACE FUNCTION public.admin_billing_series(_months integer DEFAULT 12)
RETURNS TABLE (
  month_start date,
  revenue numeric,
  paying_orgs integer,
...
REVOKE ALL ON FUNCTION public.admin_billing_series(integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.admin_billing_series(integer) TO authenticated, service_role;

-- Summary metrics (KPIs) + trial cohorts
CREATE OR REPLACE FUNCTION public.admin_billing_metrics(_months integer DEFAULT 12)
RETURNS jsonb
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
...
BEGIN
  ...
  SELECT jsonb_agg(row_to_json(t))
    INTO _series
    FROM (SELECT * FROM public.admin_billing_series(_months)) t;
  ...
  SELECT COALESCE(revenue, 0), COALESCE(paying_orgs, 0), COALESCE(new_paying_orgs, 0), COALESCE(churned_orgs, 0)
    ...
    FROM public.admin_billing_series(_months)
  ...
  -- Trial→Paid: orgs created ≥ 14 days ago in the last 90 days.
...
REVOKE ALL ON FUNCTION public.admin_billing_metrics(integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.admin_billing_metrics(integer) TO authenticated, service_role;
